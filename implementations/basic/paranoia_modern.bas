...
20 PRINT "in  BASIC  v. A1.10  (May 1982) on the  IBM  Personal Computer."
...
'Integers use less memory than decimals, saves RAM; faster performance from early computers.'
'Uncommenting the double line defines a-h,o-z as floats'
'Line 70 defines I-N as integers, by implication rest are floating point(single precision) - default'
60 '*****************************************************************
...
'Machine stores program in RAM starting from lowest memory address. Keeping utility subroutines like'
'print and error handling early in program keeps them always in range for gosub calls anywhere'
...
' Process all possible calculations and log the number that successfully run - metric of success (or not)'

250 '________ Only for  Compiled BASIC  on the  IBM PC  ________

260 V8=0 : Q8=0 : ON ERROR GOTO 270 : GOTO 380 'Q8 logs div by 0'
...
'Error line = ERL 303(Sq rt equations)'
'J = line of equation, increment. The push to edge of 64 float limit, then square, multiply and push to infinities or denormals'
'6 is an overflow error; however, compiler may avoid running and replace with infinity, or change sequence'
'Goto/resume handles error, increments q8 or v8 then heads to line immediately after the one that caused a crash'
...
'Radix = base (binary, hexadecimal, decimal; Mantissa/significant in IEEE 754 binary32 single precision P=24'
'23 explicit bits and the hidden leading bit, in double it's 52 explicit bits plus hidden bit)'
...
5390 Z=(Y-X)*O2+Y : X=Y : Y=Z 
5393 IF (O1+(X-F9)*(X-F9)>O1) THEN GOTO 5395
5395 I = I +1 : IF I <= I0 THEN GOTO 5371
5396 '----Loop end----'
...
6250 PRINT T$;"appears to be Excellent! : go to 4550
...
6540 ' S=?sticky bit, S1=... single bit set to 1 if any infinite trailing bits beyond guard, round are 1
